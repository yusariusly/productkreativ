...
import { novelSeries, sampleNovelChapters, formatNumber } from "@/data/dummy";
import { useLanguage } from "@/context/LanguageContext";
import { useAuth } from "@/context/AuthContext";
import styles from "./detail.module.css";

// Realistic novel chapter preview thumbnails
const chapterPreviews = [
  "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=200&q=80",
  ...
  "https://images.unsplash.com/photo-1476275466078-4007374efbbe?w=200&q=80",
...
export default function NovelDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const chapters = sampleNovelChapters;
        ...
        <div className={styles.heroContent}>
          ...
          <div className={styles.heroInfo}>
            <div className={styles.badges}>
              ...
              <span className="badge badge-novel">{t("novels")}</span>
            ...
            <div className={styles.actions}>
              <Link href={`/novels/${id}/1`} className="btn btn-primary btn-lg">
              ...
              <button 
                ...
                onClick={() => {
                  if (!isLoggedIn) {
                    router.push(`/login?redirect=${encodeURIComponent(`/novels/${id}?bookmark=true`)}`);
                  } else {
                    toggleBookmark(id);
                  }
                }}
          ...
          <div className={styles.chapterList}>
            {chapters.map((chapter) => {
              ...
              return (
                <Link
                  key={chapter.id}
                  href={`/novels/${id}/${chapter.number}`}
                  className={styles.chapterItem}
                  onClick={(e) => {
                    if (chapter.accessType !== "free" && !isLoggedIn) {
                      e.preventDefault();
                      router.push(`/login?redirect=/novels/${id}/${chapter.number}`);
                    }
                  }}
                >
                  {/* Novel Chapter Thumbnail Image */}
                  <div className={styles.chapterThumb}>
                    <img src={previewThumb} alt={`Thumbnail Chapter ${chapter.number}`} className={styles.chapterThumbImage} />
                    <span className={styles.chapterThumbNum}>#{chapter.number}</span>
                  </div>

                  <div className={styles.chapterInfo}>
                    <h3 className={styles.chapterTitle}>Chapter {chapter.number}</h3>
